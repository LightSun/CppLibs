...
namespace h7 {
    ...
    public:
        ...
        long long toLong(long long def = 0, int base = 10);
        unsigned long long toULong(unsigned long long def = 0, int base = 10);
...
//----------------------
    ...
    StringRef::StringRef(u32 len, bool resize){
        alloc0(len + 1);
    ...
    StringRef::StringRef(char* data, u32 start, u32 len){
        m_data = data + start;
        m_validLen = len;
    }
    StringRef::StringRef(const char* data, u32 start, u32 len){
        u32 old = m_validLen;
    ...
    StringRef::StringRef(const String& ref){
        m_validLen = ref.length();
        alloc0(m_validLen + 1);
        memcpy(m_data, ref.data(), m_validLen + 1);
        m_selfOwned = true;
    }
    StringRef::StringRef(String* ref){
    ...
    void StringRef::prepareSize(u32 size){
         alloc0(size + 1);
    }
    void StringRef::resize(u32 size){
        prepareSize(size);
        m_validLen = size;
    }

    StringRef StringRef::subString(u32 start, u32 end)const{
        if(m_data == nullptr){
    ...
    bool StringRef::startsWith(const StringRef& ref)const{
        if(m_validLen < ref.m_validLen){
            return false;
        }
        return strncmp(m_data, ref.m_data, ref.m_validLen) == 0;
    }
    bool StringRef::endsWith(const StringRef& ref)const{
        if(m_validLen < ref.m_validLen){
            return false;
        }
        // 123  23
    ...
    StringRef StringRef::reverse()const{
    ...
    int StringRef::indexOf(char c, int fromIndex)const{
    ...
    int StringRef::lastIndexOf(char c, int fromIndex)const{
        if(fromIndex == -1){
    ...
    int StringRef::indexOf(const StringRef& ref, int fromIndex)const{
        if(fromIndex < 0){
            return -1;
        }
    ...
    int StringRef::lastIndexOf(const StringRef& ref, int fromIndex)const{
        if(fromIndex == -1){
            fromIndex = m_validLen -1;
        }
        return indexOf(ref, fromIndex - ref.m_validLen + 1);
    }
    StringRef& StringRef::append(const StringRef& ref){
        int old = m_validLen;
        m_validLen += ref.m_validLen;
        alloc0(m_validLen + 1);
    ...
    //------------------------
    unsigned long long StringRef::toULong(unsigned long long def, int base){
        auto str = toString();
        try{
    ...
    long long StringRef::toLong(long long def, int base){
        auto str = toString();
        try{
    ...
    bool StringRef::toBool(){
        return equals("true") || equals("TRUE");
    }
    int StringRef::toInt(int def, int base){
        auto str = toString();
        try{
    ...
    float StringRef::toFloat(float def){
        auto str = toString();
        try{
               return std::stof(str);
           }catch (std::invalid_argument&){
    ...
    float StringRef::toDouble(float def){
        auto str = toString();
        try{
               return std::stod(str);
           }catch (std::invalid_argument&){
    ...
    bool StringRef::equals(const StringRef& ref)const{
        if(m_validLen != ref.m_validLen){
            return false;
        }
        return strncmp(m_data, ref.m_data, m_validLen) == 0;
    }
    bool StringRef::equalsIgnoreCase(const StringRef& ref)const{
        if(m_validLen != ref.m_validLen){
            return false;
        }
        for(u32 i = 0 ; i < m_validLen ; ++i){
...
//        return s1.equals(s2);
    ...
    int StringRef::compareTo(const StringRef& oth)const{
        int len1 = m_validLen;
