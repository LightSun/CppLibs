...
namespace h7 {
    ...
    public:
        ...
        ByteBuffer(const void* data, size_t size, size_t valid_size){
            m_buffer.resize(size);
            if(valid_size > 0){
                memcpy(m_buffer.data(), data, valid_size);
                m_dataSize = valid_size;
            }
        }
        int capacity(){
        ...
        String getString(){
            int len = getInt();
            printf("getString: len = %d\n", len);
            std::vector<char> data(len, 0);
            char* src = (char*)data_ptr() + m_pos;
        ...
        void markDataSizeToHead(){
            int dsize = m_dataSize - sizeof (int);
            memcpy((void*)data_ptr(), &dsize, sizeof (int));
        }
        void flushTo(void* dst){
            if(m_dataSize > 0){
                memcpy(dst, m_buffer.data() + m_pos, m_dataSize);
            }
        }
    private:
