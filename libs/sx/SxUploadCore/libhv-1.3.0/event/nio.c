...
static int __nio_read(hio_t* io, void* buf, int len) {
    ...
    printf("__nio_read >> size =%d\n", nread);
    return nread;
}

static int __nio_write0(hio_t* io, const void* buf, int len) {
    int nwrite = 0;
    switch (io->io_type) {
    case HIO_TYPE_SSL:
    ...
    //hlogw
    //printf("----> __nio_write0: nwrite = %d, req_size = %d\n", nwrite, len);
    return nwrite;
}

static int __nio_write(hio_t* io, const void* buf, int len) {
    return __nio_write0(io, buf, len);
   /* int write_size = __nio_write0(io, buf, len);
    if(write_size <= 0){
        return write_size;
    }
    char* nbuf = (char*)buf;
    int offset = write_size;
    while(offset < len){
        write_size = __nio_write0(io, nbuf + offset, len - offset);
        if(write_size > 0){
            offset += write_size;
        }
    }
   // printf("end send data: len = %d\n", len);
    return len;
    */
}
...
int hio_connect(hio_t* io) {
    ...
    int err_code = socket_errno();
#ifdef OS_WIN
    if (ret < 0 && err_code != WSAEWOULDBLOCK) {
#else
    if (ret < 0 && err_code != EINPROGRESS) {
#endif
        perror("connect");
        io->error = socket_errno();
        hio_close_async(io);
...
try_write:
        nwrite = __nio_write(io, buf, len);
        //printf("__nio_write: ---> write retval=%d\n", nwrite);
        if (nwrite < 0) {
            err = socket_errno();
        ...
        if (nwrite == 0) {
            hlogw("nwrite = 0, goto disconnect!");
            goto disconnect;
        }
        if (nwrite == len) {
            goto write_done;
