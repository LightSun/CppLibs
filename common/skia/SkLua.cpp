...
static SkRect* lua2rect(lua_State* L, int index, SkRect* rect) {
    SkPoint leftTop = {getfield_scalar_default(L, index, "left", 0), getfield_scalar_default(L, index, "top", 0)};
    SkPoint rightBottom = {getfield_scalar(L, index, "right"), getfield_scalar(L, index, "bottom")};
...
static int lcanvas_drawImage(lua_State* L) {
    ...
    SkPaint paint;
    canvas->drawImage(image, x, y, SkSamplingOptions(), lua2OptionalPaint(L, 5, &paint));
    return 0;
}
...
static int lcanvas_drawImageRect(lua_State* L) {
    ...
    SkPaint paint;
    canvas->drawImageRect(image, *srcRPtr, dstR, SkSamplingOptions(),
                          lua2OptionalPaint(L, 5, &paint), SkCanvas::kFast_SrcRectConstraint);
    return 0;
}
...
static int lcanvas_drawText(lua_State* L) {
    ...
    if (lua_isstring(L, 2) && lua_isnumber(L, 3) && lua_isnumber(L, 4)) {
        ...
        //sk_sp<SkTextBlob> sp =SkTextBlob::MakeFromString(text, SkFont());
        SkString str(text);
        get_ref<SkCanvas>(L, 1)->drawString(str, lua2scalar(L, 3), lua2scalar(L, 4),
                                              SkFont(), *get_obj<SkPaint>(L, 5));
...
static int limage_newShader(lua_State* L) {
    ...
    push_ref(L, get_ref<SkImage>(L, 1)->makeShader(tmode, tmode, SkSamplingOptions(), localM));
    return 1;
}
...
static int lsk_newDocumentPDF(lua_State* L) {
   ...
   // SkFILEWStream file(filename);
    ...
    sk_sp<SkDocument> doc = SkPDF::MakeDocument(
           file.get(), metadata);
    if (!doc) {
        return 0;
    }
